...
describe('VariablesHandler unit tests', () => {
  ...
  describe('getVariableAttributesByVariableRef - when SCOPES.all.ref is requested', () => {
    it('getVariableAttributesByVariableRef should pass when variables contain array property', async () => {
      ...
      // Assert
      const containsArrayProp = result.some((e) =>
        e.name === arrayProp.key && e.value === JSON.stringify(arrayProp.value) && e.variablesReference === 0);
    ...
    it('getVariableAttributesByVariableRef should pass when variables contain boolean property', async () => {
      ...
      const result = await variablesHandler.getVariableAttributesByVariableRef(SCOPES.all.ref);
      // Assert
      const containsBoolProp = result.some((e) =>
        e.name === boolProp.key && e.value === JSON.stringify(boolProp.value) && e.variablesReference === 0);
    ...
    it('getVariableAttributesByVariableRef should pass when variables contain null property', async () => {
      ...
      const result = await variablesHandler.getVariableAttributesByVariableRef(SCOPES.all.ref);
      // Assert
      const containsNullProp = result.some((e) =>
        e.name === nullProp.key && e.value === JSON.stringify(nullProp.value) && e.variablesReference === 0);
    ...
    it('getVariableAttributesByVariableRef should pass when variables contain number property', async () => {
      ...
      const result = await variablesHandler.getVariableAttributesByVariableRef(SCOPES.all.ref);
      // Assert
      const containsNumberProp = result.some((e) =>
        e.name === numProp.key && e.value === JSON.stringify(numProp.value) && e.variablesReference === 0);
    ...
    it('getVariableAttributesByVariableRef should pass when variables contain object property', async () => {
      ...
      assert.strictEqual(objectVar!.value, OBJECT_VARIABLE_DISPLAY_NAME,
        `objectVar\'s value should be ${OBJECT_VARIABLE_DISPLAY_NAME}`);
      assert.strictEqual(generateVariablesAttrKeyStub.calledWith('', objProp.key), true,
        'generateVariablesAttrKey should be called with concrete arguments');
    });

    it('getVariableAttributesByVariableRef should call handles.create when variables contain object property',
      async () => {
        // Arrange
        sinon.stub(RuntimeInterface.prototype, 'variables').resolves({ [objProp.key]: objProp.value });
        ...
        assert.strictEqual(objectVar!.value, OBJECT_VARIABLE_DISPLAY_NAME,
          `objectVar\'s value should be ${OBJECT_VARIABLE_DISPLAY_NAME}`);
        assert.strictEqual(handlesCreateStub.calledWithMatch(variablePath), true,
    ...
    it('getVariableAttributesByVariableRef should pass when variables contain string property', async () => {
      ...
      const result = await variablesHandler.getVariableAttributesByVariableRef(SCOPES.all.ref);
      // Assert
      const containsStringProp = result.some((e) =>
        e.name === stringProp.key && e.value === JSON.stringify(stringProp.value) && e.variablesReference === 0);
    ...
    it('getVariableAttributesByVariableRef should pass when variables contain undefined property', async () => {
      ...
      const result = await variablesHandler.getVariableAttributesByVariableRef(SCOPES.all.ref);
      // Assert
      const containsUndefinedProp = result.some((e) =>
  ...
  it('getVariableAttributesByVariableRef should pass when variableReference is not SCOPE.all.ref',
    async () => {
      // Arrange
      ...
      sinon.stub(Handles.prototype, 'get')
      ...
      const containsNestedObjectProperty = result.some((v) =>
  ...
  describe('evaluateExpression', () => {
    it('should pass when expression is simple', async () => {
      ...
      // Assert
      assert.strictEqual(evaluatedVar.result, JSON.stringify(variables[variableKey]),
