...
describe('RuntimeInterface unit tests', () => {
  ...
  beforeEach(async () => {
    sinon.stub(contractsPrepareHelpers, 'prepareContracts')
  ...
  it('callStack should pass when session.view(evm.current.callstack) contains one call',
  async () => {
    // Arrange
    ...
    sinon.stub(RuntimeInterface.prototype, 'currentLine')
      .returns({ column: 1, file: contractSourcePathMock, line: 1 });
    ...
    // Assert
    assert.strictEqual(callStack[0].column, currentDebugLine.column,
      assertCallstackMessage('column', currentDebugLine.column));
    assert.strictEqual(callStack[0].file, currentDebugLine.file,
      assertCallstackMessage('file', currentDebugLine.file));
    assert.strictEqual(callStack[0].line, currentDebugLine.line,
      assertCallstackMessage('line', currentDebugLine.line));
  });
