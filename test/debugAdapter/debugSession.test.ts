...
import {
  ...
  GET_INSTRUCTIONS,
} from '../../src/debugAdapter/constants/debugSessionCommands';
...
describe('DebugSession unit tests', () => {
  ...
  it('shouldn\'t contain vscode module as a dependency', (done) => {
    ...
    debugSessionResolvingProcess.stderr.on('data', (data: string | Buffer) => {
      ...
      assert.strictEqual(isContainVscodeModuleNotFoundError, false,
  ...
  it('should send DebuggerTypes.LaunchedEvent and StoppedEvent in specific order', async () => {
    ...
    // Assert
    assert.strictEqual(
      sendLaunchedEventStub.calledWith(launchedEvent), true, 'sendLaunchedEventStub should be called with argument');
    assert.strictEqual(
      sendStoppedEventStub.calledWith(stoppedEvent), true, 'sendStoppedEventStub should be called with argument');
    assert.strictEqual(sendStoppedEventStub.calledAfter(sendLaunchedEventStub), true,
  ...
  describe('should send sendErrorResponse when something goes wrong', () => {
    ...
    beforeEach(() => {
      sendErrorResponseStub = sinon.stub(SolidityDebugSession.prototype, 'sendErrorResponse' as any)
        .returns(void 0);
  ...
  it('setBreakPointsRequest should store breakPoints before debugger is attached',
    async () => {
      // Arrange
  ...
  describe('customRequest', () => {
    ...
    beforeEach(() => {
      ...
      getAllInstructionsStub = sinon.stub(RuntimeInterface.prototype, 'getInstructionSteps')
        .returns(instructionsMock);
      getCurrentInstructionStub = sinon.stub(RuntimeInterface.prototype, 'getCurrentInstructionStep')
    ...
    it('should return response with all instructions when command is GET_INSTRUCTIONS', async () => {
      ...
      assert.strictEqual(sendResponseStub.calledWith(response), true,
