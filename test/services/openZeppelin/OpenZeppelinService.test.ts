...
describe('OpenZeppelinService', () => {
  ...
  beforeEach(() => {
    ...
    getConfigurationAsyncStub = sinon.stub(helpers.userSettings, 'getConfigurationAsync')
  ...
  it('downloadAssetsAsync should make file readonly', async () => {
    ...
    const testAssets = [
      { id: '1', name: 'assetA' } as IOZAsset,
      { id: '2', name: 'assetB' } as IOZAsset,
    ];

    // Act
    await openZeppelinServiceRewire.OpenZeppelinService
      .downloadAssetsAsync('http://test.com', testAssets, true, 'destFolder');

    // Assert
    assert
      .strictEqual(fsChmodStub.callCount, testAssets.length, `fs.chmod should be called ${testAssets.length} times`);
  });
  ...
  it('updateOpenZeppelinContractsAsync should update contracts in user project', async () => {
    ...
    const newAssets = [
      { id: '1', name: 'A', dependencies: ['2'] } as IOZAsset,
      { id: '2', name: 'B' } as IOZAsset,
    ];
    ...
    // Assert
    assert.strictEqual(createNewProjectJsonAsyncStub.calledOnce, true,
      'createNewProjectJsonAsync should be called once');
    assert.strictEqual(moveFolderStub.calledTwice, true,
      'moveFolder should be called twice');
    assert.strictEqual(moveProjectJsonAsyncStub.calledTwice, true,
      'moveProjectJsonAsync should be called twice');
  ...
  describe('getCurrentOpenZeppelinVersionAsync', () => {
    ...
    beforeEach(() => {
      ...
      sinon.stub(OpenZeppelinProjectJsonService, 'getProjectJson').resolves(
        { openZeppelin: { version: testProjectJsonVersion } } as IProjectMetadata);

      constants.Constants.allOpenZeppelinVersions =
        ['1.0.0', testUserSettingsVersion.userValue, testUserSettingsVersion.defaultValue];
    });
    ...
    it('should return user version from userSettings when project.json doesn\'t exist', async () => {
      ...
      // Arrange
      assert.strictEqual(currentVersion, testUserSettingsVersion.userValue, `currentVersion should be ${testUserSettingsVersion.userValue}`);
    });
    ...
    it('should return default version from userSettings when project.json doesn\'t exist and user version is not defined', async () => {
      ...
      // Assert
      assert.strictEqual(currentVersion, testUserSettingsVersion.defaultValue, `currentVersion should be ${testUserSettingsVersion.defaultValue}`);
    });
  });
  ...
  it('getLatestOpenZeppelinVersionAsync should return defaultValue from userSettings', async () => {
    ...
    // Assert
    assert.strictEqual(latestVersion, testUserSettingsVersion.defaultValue, `currentVersion should be ${testUserSettingsVersion.defaultValue}`);
  });
});
