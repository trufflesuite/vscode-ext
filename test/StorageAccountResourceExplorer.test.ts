...
describe("Storage Account Resource Explorer", () => {
  ...
  it("getFileBlobUrls should executed all methods for delete BDM application", async () => {
    ...
    // Act
    await storageAccountResourceExplorer.prototype.getFileBlobUrls(
      contentArray,
      fileNameArray,
      localFilePaths,
      subscriptionId,
    ...
    assert.strictEqual(
      createStorageAccountIfDoesNotExistStub.calledOnce,
      true,
      "createStorageAccountIfDoesNotExist should be called"
    );
    assert.strictEqual(getStorageAccountSasStub.calledOnce, true, "getStorageAccountSas should be called");
    assert.strictEqual(
      createContainerIfDoesNotExistStub.calledOnce,
      true,
      "createContainerIfDoesNotExist should be called"
    );
    assert.strictEqual(
      createBlobStub.callCount,
      fileNameArray.length,
      `createBlob should be called ${fileNameArray.length} times`
    );
  });
  ...
  it("createStorageAccountIfDoesNotExist should check existing storage account", async () => {
    ...
    // Act
    await storageAccountResourceExplorer.prototype.createStorageAccountIfDoesNotExist(
      client,
      location,
      storageAccountName
    );
  ...
  it(
    "createStorageAccountIfDoesNotExist should wait when storage account will be created " +
      "when provisioning state equal creating",
    async () => {
      // Arrange
      const client = {
        storageResource: {getStorageAccount: () => Promise.resolve({})},
  ...
  it(
    "createStorageAccountIfDoesNotExist should wait when storage account will be created " +
      "when provisioning state equal resolvingDns",
    async () => {
      // Arrange
      const client = {
        storageResource: {getStorageAccount: () => Promise.resolve({})},
  ...
  it("createStorageAccount should create new storage account", async () => {
    ...
    assert.strictEqual(
      typeof awaiterStub.args[0][0],
      "function",
      "awaiter should be called called with first parameter like function"
    );
  });
  ...
  it("createBlob should create blob to Azure and create file in build directory", async () => {
    ...
    // Act
    await storageAccountResourceExplorer.prototype.createBlob(
      uuid.v4(),
      uuid.v4(),
      uuid.v4(),
      uuid.v4(),
