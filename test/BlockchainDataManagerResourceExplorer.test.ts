...
describe("Blockchain Data Manager Resource Explorer", () => {
  ...
  describe("Public methods", () => {
    ...
    beforeEach(() => {
      ...
      getOrSelectSubscriptionItemStub = sinon
        .stub(blockchainDataManagerResourceExplorer.prototype, "getOrSelectSubscriptionItem")
        .returns(Promise.resolve(subItemTest));
      getOrCreateResourceGroupItemStub = sinon
        .stub(blockchainDataManagerResourceExplorer.prototype, "getOrCreateResourceGroupItem")
        .returns(Promise.resolve(rgItemTest));
      getBlockchainDataManagerInstanceStub = sinon.stub(
        blockchainDataManagerResourceExplorer.prototype,
        "getBlockchainDataManagerInstance"
      );
    });
    ...
    it("selectProject should return blockchain data manager instance", async () => {
      ...
      assert.strictEqual(
        getOrCreateResourceGroupItemStub.calledOnce,
        true,
        "getOrCreateResourceGroupItem should be called"
      );
    ...
    it("deleteBDMApplication should executed all methods for delete BDM application", async () => {
      ...
      const deleteBlobsStub = sinon
        .stub(StorageAccountResourceExplorer.prototype, "deleteBlobs")
        .returns(Promise.resolve());
      const removeItemStub = sinon.stub(TreeManager, "removeItem");
      const deleteBlockchainDataManagerApplicationSpy = sinon.spy(
        azureExplorer.bdmResource,
        "deleteBlockchainDataManagerApplication"
      );
      sinon.stub(ContractService, "getBuildFolderPath");
      sinon
        .stub(blockchainDataManagerResourceExplorer.prototype, "getAzureClient")
      ...
      // Act
      await blockchainDataManagerResourceExplorer.prototype.deleteBDMApplication(
        bdmLabel,
        application,
        new StorageAccountResourceExplorer()
      );

      // Assert
      assert.strictEqual(
        deleteBlockchainDataManagerApplicationSpy.calledOnce,
        true,
        "deleteBlockchainDataManagerApplication should be called"
      );
    ...
    describe("createNewBDMApplication", () => {
      ...
      beforeEach(() => {
        ...
        getSolidityContractsFolderPathStub = sinon
          ...
          .returns(Promise.resolve(""));
        getDeployedBytecodeByAddressStub = sinon
          .stub(ContractService, "getDeployedBytecodeByAddress")
          .returns(Promise.resolve(uuid.v4()));
        getBDMApplicationNameStub = sinon
          .stub(blockchainDataManagerResourceExplorer.prototype, "getBDMApplicationName")
          .returns(Promise.resolve(uuid.v4()));
        getBlobUrlsStub = sinon
          .stub(blockchainDataManagerResourceExplorer.prototype, "getBlobUrls")
      ...
      it("should executed all methods for delete BDM application", async () => {
        ...
        // Act
        await blockchainDataManagerResourceExplorer.prototype.createNewBDMApplication(
        ...
        assert.strictEqual(
          ...
          true,
          "getSolidityContractsFolderPath should be called"
        );
        assert.strictEqual(countSolFiles, expectedSolFiles, "showQuickPick should show only solidity files");
        assert.strictEqual(
          getDeployedBytecodeByAddressStub.calledOnce,
          true,
          "getDeployedBytecodeByAddressStub should be called"
        );
      ...
      it("throws error when contract directory does not have solidity files", async () => {
        ...
        try {
          ...
          assert.strictEqual(
            getDeployedBytecodeByAddressStub.calledOnce,
            false,
            "getDeployedBytecodeByAddressStub should not called"
          );
    ...
    describe("createProject", () => {
      ...
      beforeEach(() => {
        ...
        getSelectedMemberStub = sinon
          .stub(blockchainDataManagerResourceExplorer.prototype, "getSelectedMember")
          .returns(Promise.resolve(selectedMember));
        getSelectedTransactionNodeStub = sinon
          .stub(blockchainDataManagerResourceExplorer.prototype, "getSelectedTransactionNode")
          .returns(new TransactionNodeItem(transactionName, uuid.v4(), uuid.v4()));
        getSelectedEventGridStub = sinon
          .stub(blockchainDataManagerResourceExplorer.prototype, "getSelectedEventGrid")
          .returns(selectedEventGrid);
        createBlockchainDataManagerInstanceStub = sinon.stub(
          blockchainDataManagerResourceExplorer.prototype,
      ...
      it("all method should be executed for create Blockchain Data Manager", async () => {
        ...
        const getSelectedConsortiumStub = sinon
          .stub(blockchainDataManagerResourceExplorer.prototype, "getSelectedConsortium")
        ...
        assert.strictEqual(
          getSelectedMemberStub.args[0][2],
          memberName,
          "getSelectedMember should be called with special member name"
        );
        assert.strictEqual(
          getBlockchainDataManagerListSpy.calledOnce,
          true,
          "getBlockchainDataManagerList should be called"
        );
        assert.strictEqual(
          getSelectedTransactionNodeStub.calledOnce,
          true,
          "getSelectedTransactionNode should be called"
        );
        assert.strictEqual(
          getSelectedTransactionNodeStub.args[0][1],
          bdmList,
          "getSelectedTransactionNode should be called with special bdm list"
        );
        assert.strictEqual(
        ...
        assert.strictEqual(
          createBlockchainDataManagerInstanceStub.calledOnce,
          true,
          "createBlockchainDataManagerInstance should be called"
        );
        assert.strictEqual(
      ...
      it("should run the creation of a consortium", async () => {
        ...
        try {
          // Act
          await blockchainDataManagerResourceExplorer.prototype.createProject(
          ...
          assert.strictEqual(
            createConsortiumCallbackStub.calledOnce,
            true,
            "createConsortiumCallback should be called"
          );
          ...
          assert.strictEqual(
            getBlockchainDataManagerListSpy.notCalled,
            true,
            "getBlockchainDataManagerList should not called"
          );
          assert.strictEqual(
            getSelectedTransactionNodeStub.notCalled,
            true,
            "getSelectedTransactionNode should not called"
          );
          assert.strictEqual(getSelectedEventGridStub.notCalled, true, "getSelectedEventGrid should not called");
          assert.strictEqual(
            createBlockchainDataManagerInstanceStub.notCalled,
            true,
            "createBlockchainDataManagerInstance should not called"
          );
        }
      });
    });
  ...
  describe("Private methods", () => {
    it("loadBlockchainDataManagerInstanceItems should return bdm instances", async () => {
      ...
      // Act
      const result = await blockchainDataManagerResourceExplorer.prototype.loadBlockchainDataManagerInstanceItems(
        azureExplorer,
        [bdmList[0].name]
      );
    ...
    it("getBlockchainDataManagerInstance should return bdm instance", async () => {
      ...
      const getBlockchainDataManagerApplicationListSpy = sinon.spy(
        azureExplorer.bdmResource,
        "getBlockchainDataManagerApplicationList"
      );
      const getBlockchainDataManagerInputListSpy = sinon.spy(
        azureExplorer.bdmResource,
        "getBlockchainDataManagerInputList"
      );
      const getBlockchainDataManagerOutputListSpy = sinon.spy(
        azureExplorer.bdmResource,
        "getBlockchainDataManagerOutputList"
      );

      // Act
      const bdmProject = await blockchainDataManagerResourceExplorer.prototype.getBlockchainDataManagerInstance(
        blockchainDataManagerInstanceItem,
        azureExplorer
      );
      ...
      assert.strictEqual(
        bdmProject.children.length,
        applicationList.length + 2,
        "result should has special number of children"
      );
      assert.strictEqual(
        bdmProject.children.filter((ch: BlockchainDataManagerNetworkNode) => ch.label === input.label)[0].children
          .length,
        inputList.length,
        "Input group should has special number of inputs"
      );
      assert.strictEqual(
        bdmProject.children.filter((ch: BlockchainDataManagerNetworkNode) => ch.label === output.label)[0].children
          .length,
        outputList.length,
      ...
      assert.strictEqual(
        getBlockchainDataManagerInputListSpy.calledOnce,
        true,
        "getBlockchainDataManagerInputList should be called"
      );
      assert.strictEqual(
        getBlockchainDataManagerOutputListSpy.calledOnce,
        true,
        "getBlockchainDataManagerOutputList should be called"
      );
    });
    ...
    it("createBlockchainDataManagerInstance should return bdm instance", async () => {
      ...
      const getBlockchainDataManagerConnectionNameStub = sinon
        .stub(blockchainDataManagerResourceExplorer.prototype, "getBlockchainDataManagerConnectionName")
        .returns(Promise.resolve(connectionName));
      const getBlockchainDataManagerNameStub = sinon
        .stub(blockchainDataManagerResourceExplorer.prototype, "getBlockchainDataManagerName")
      ...
      const createBlockchainDataManagerInputSpy = sinon.spy(
        azureExplorer.bdmResource,
        "createBlockchainDataManagerInput"
      );
      const createBlockchainDataManagerOutputSpy = sinon.spy(
        azureExplorer.bdmResource,
        "createBlockchainDataManagerOutput"
      );
      const startBlockchainDataManagerStub = sinon.stub(
        blockchainDataManagerResourceExplorer.prototype,
        "startBlockchainDataManager"
      );
      ...
      assert.strictEqual(
        bdmProject.children.length,
        expectedNumberChildren,
        `result should has only ${expectedNumberChildren} children`
      );
      assert.strictEqual(
      ...
      assert.strictEqual(
        getBlockchainDataManagerNameStub.calledOnce,
        true,
        "getBlockchainDataManagerName should be called"
      );
      assert.strictEqual(withProgressStub.calledOnce, true, "withProgress should be called");
      assert.strictEqual(
        createBlockchainDataManagerSpy.calledOnce,
        true,
        "createBlockchainDataManager should be called"
      );
      assert.strictEqual(
      ...
      assert.strictEqual(
        createBlockchainDataManagerInputSpy.calledOnce,
        true,
        "createBlockchainDataManagerInput should be called"
      );
      assert.strictEqual(
        createBlockchainDataManagerInputSpy.args[0][0],
        selectedBdmName,
        "createBlockchainDataManagerInput should be called with special bdm name"
      );
      assert.strictEqual(
        createBlockchainDataManagerInputSpy.args[0][1],
        transactionNodeName,
        "createBlockchainDataManagerInput should be called with special transaction node name"
      );
      assert.strictEqual(
        createBlockchainDataManagerOutputSpy.calledOnce,
        true,
        "createBlockchainDataManagerOutput should be called"
      );
      assert.strictEqual(
        createBlockchainDataManagerOutputSpy.args[0][0],
        selectedBdmName,
        "createBlockchainDataManagerOutput should be called with special bdm name"
      );
      assert.strictEqual(
        createBlockchainDataManagerOutputSpy.args[0][1],
        connectionName,
        "createBlockchainDataManagerOutput should be called with special connection name"
      );
      assert.strictEqual(
        startBlockchainDataManagerStub.calledOnce,
        true,
        "startBlockchainDataManager should be called"
      );
      assert.strictEqual(
        startBlockchainDataManagerStub.args[0][1],
        bdmItems[0].id,
        "startBlockchainDataManager should be called with selected bdm url"
      );
      assert.strictEqual(
        startBlockchainDataManagerStub.args[0][2],
        selectedBdmName,
      ...
      describe("should return error when connection name does not correct", () => {
        ...
        incorrectConnectionName.forEach((name) => {
          it(`and equal ${name}`, async () => {
            ...
            // Act
            const result =
              await blockchainDataManagerResourceExplorer.prototype.getBlockchainDataManagerConnectionName();

            // Assert
            assert.strictEqual(
              result,
              Constants.validationMessages.forbiddenChars.outboundConnectionName,
              "result should has error"
            );
          });
        });
      });
      ...
      describe("should return error when bdm name does not correct", () => {
        ...
        incorrectConnectionName.forEach((name) => {
          it(`and equal ${name}`, async () => {
            ...
            // Act
            const result = await blockchainDataManagerResourceExplorer.prototype.getBlockchainDataManagerName(bdmItems);

            // Assert
            assert.strictEqual(
              result,
              Constants.validationMessages.invalidBlockchainDataManagerName,
              "result should has error"
            );
          });
        });
    ...
    describe("getTransactionNodeName", () => {
      ...
      describe("should return error when transaction node name does not correct", () => {
        ...
        incorrectConnectionName.forEach((name) => {
          it(`and equal ${name}`, async () => {
            ...
            // Act
            const result = await blockchainDataManagerResourceExplorer.prototype.getTransactionNodeName(
              transactionNodeItems
            );
      ...
      it("should return error when transaction node name already exist", async () => {
        ...
        assert.strictEqual(
          result,
          Constants.validationMessages.transactionNodeNameAlreadyExists,
          "result should has error"
        );
      });
    ...
    describe("getEventGridName", () => {
      ...
      beforeEach(() => {
        eventGridItems = [new EventGridItem("eventgrid1", uuid.v4()), new EventGridItem("eventgrid2", uuid.v4())];
      });
      ...
      describe("should return error when event grid name does not correct", () => {
        ...
        incorrectConnectionName.forEach((name) => {
          it(`and equal ${name}`, async () => {
            ...
            // Act
            const result = await blockchainDataManagerResourceExplorer.prototype.getEventGridName(eventGridItems);
      ...
      describe("should not returns error when event grid name correct", () => {
        const correctConnectionName = [
          ...
          "AAA",
          "aA-1",
          "aaaaaaaaaa1111111111aaaaaaaaaa1111111111aaaaaaaaaa",
        ];
        correctConnectionName.forEach((name) => {
          it(`and equal ${name}`, async () => {
            // Arrange
    ...
    describe("getSelectedConsortium", () => {
      ...
      it("should return quick pick with available consortia", async () => {
        ...
        const expectedNumberConsortia = consortiaItemList.filter((con) =>
          Constants.availableBlockchainDataManagerLocations.includes(con.location)
        ).length;

        // Act
        await blockchainDataManagerResourceExplorer.prototype.getSelectedConsortium(
          consortiumResourceExplorer,
          azureExplorer
        );
        ...
        assert.strictEqual(
          ...
          expectedNumberConsortia,
          `QuickPick should has ${expectedNumberConsortia} consortia item`
        );
      });
      ...
      it("should return quick pick with add button", async () => {
        // Arrange
        const consortiumResourceExplorer = {
          loadConsortiumItems: async () =>
            consortiaItemList.filter(
        ...
        assert.strictEqual(
          expectedItem.label,
          Constants.uiCommandStrings.createConsortium,
          "QuickPick should has add button item"
        );
      });
    });
    ...
    describe("getSelectedTransactionNode", () => {
      ...
      beforeEach(() => {
        ...
        expectedQuickPickItems =
          transactionNodeList.filter(
            (tn) =>
              tn.properties.provisioningState === Constants.provisioningState.succeeded &&
              !bdmInputList.some((input) => input.properties.dataSource.resourceId === tn.id)
          ).length + 2;
        ...
        getBlockchainDataManagerInputListSpy = sinon.spy(
          azureExplorer.bdmResource,
          "getBlockchainDataManagerInputList"
        );
      ...
      it("should return selected transaction node", async () => {
        ...
        const createdTransactionNodeStub = sinon.stub(
          blockchainDataManagerResourceExplorer.prototype,
          "createdTransactionNode"
        );

        // Act
        await blockchainDataManagerResourceExplorer.prototype.getSelectedTransactionNode(
          azureExplorer,
          bdmItems,
          selectedMemberName,
          location
    ...
    describe("getSelectedEventGrid", () => {
      ...
      beforeEach(() => {
        ...
        expectedQuickPickItems =
          eventGridList.filter((eg) => eg.properties.provisioningState === Constants.provisioningState.succeeded)
            .length + 1;
      });
