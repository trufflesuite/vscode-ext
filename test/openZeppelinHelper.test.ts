...
describe('OpenZeppelinHelper', () => {
  ...
  it ('createManifestAsync should create instance of OpenZeppelinManifest class', async () => {
    ...
    assert.strictEqual(manifest.getCategories(), testMetadata.categories, 'manifest.getCategories should be equal categories');
    assert.strictEqual(manifest.getVersion(), testMetadata.openZeppelinVersion, 'manifest.getVersion should be equal openZeppelinVersion');
  });
