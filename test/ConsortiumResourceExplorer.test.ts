...
describe("Consortium Resource Explorer", () => {
  ...
  showQuickPickStub.onCall(0).returns(subscriptionItem);
  showQuickPickStub.onCall(1).returns(resourceGroupItem as ResourceGroupItem);
  showQuickPickStub.onCall(2).returns(consortiumItem as ConsortiumItem);
  ...
  it("selectProject all method should be executed", async () => {
    ...
    sinon.stub(consortiumResourceExplorer.prototype, "getSubscriptionItems").returns(Promise.resolve(subItemTest));
    sinon.stub(consortiumResourceExplorer.prototype, "getResourceGroupItems").returns(Promise.resolve(rgItemTest));
  ...
  it("loadConsortiumItems returns unselected consortia", async () => {
    ...
    const expectedNumberOfConsortia = (await consortiaList).filter(
      (consortium) => !excludedItemsTest.includes(consortium.consortium)
    ).length;
    ...
    // Act
    const result = await consortiumResourceExplorer.prototype.loadConsortiumItems(azureClientMock, excludedItemsTest);
  ...
  it("loadTransactionNodeItems returns empty array", async () => {
    ...
    const consortiumResourceExplorerRequire = require("../src/resourceExplorers/ConsortiumResourceExplorer");
    const consortiumResourceExplorer = consortiumResourceExplorerRequire.ConsortiumResourceExplorer;
    sinon.stub(consortiumResourceExplorer.prototype, "getAzureClient").returns({
      transactionNodeResource: {
        getTransactionNodeList: () => {
          throw Error();
        },
      },
    });
  ...
  it("getAzureConsortium returns consortium with member which has transaction nodes", async () => {
    ...
    const expectedNumberOfMembers = (await getMemberList()).filter(
      (mem: IAzureConsortiumMemberDto) => mem.status === Constants.consortiumMemberStatuses.ready
    ).length;
    // We should remember about default transaction node, because of it we use + 1
    const expectedNumberOfTransactionNodes = (await getTransactionNodeList()).length + 1;

    // Act
    const result = await consortiumResourceExplorer.prototype.getAzureConsortium(azureClient, subItemTest, rgItemTest);
    ...
    assert.strictEqual(
      ...
      `Consortium should have ${expectedNumberOfMembers} members.`
    );
    assert.strictEqual(
      transactionNodes.length,
      expectedNumberOfTransactionNodes,
      `Member should have ${expectedNumberOfTransactionNodes} transaction nodes.`
    );
  });
