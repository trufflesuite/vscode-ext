...
export class EventGridManagementClient extends BaseClient {
  ...
  public getEventGridItem(eventGridName: string, callback: (error: Error | null, result?: any) => void)
  : Promise<void> {
  ...
  private getUrl(mainPartOfUrl: string): string {
    return `${this.baseUri}/subscriptions/${this.subscriptionId}/resourceGroups/${this.resourceGroup}/providers` +
