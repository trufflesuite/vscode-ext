...
async function downloadOZFiles(
  ...
  if (existing.length > 0) {
    ...
    if (answer === Constants.openZeppelin.replaceButtonTitle) {
      Output.outputLine(Constants.outputChannel.truffleForVSCode, Constants.openZeppelin.overwriteExistedContracts);
  ...
  async function downloadFileSet(_assets: IOZAsset[]): Promise<IOZAsset[]> {
    ...
    if (rejected.length > 0) {
      ...
      if (answer === Constants.openZeppelin.retryButtonTitle) {
        Output.outputLine(Constants.outputChannel.truffleForVSCode, Constants.openZeppelin.retryDownloading);
