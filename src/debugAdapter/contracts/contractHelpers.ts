...
export namespace ContractHelper {
  ...
  export async function prepare(workingDirectory: any): Promise<ContractData> {
    ...
    const mappedSources = await getSourcePath(workingDirectory, compilations[0].sources);
   ...
   */
  async function getSourcePath(workingDirectory: string, sources: Source[]): Promise<Map<string, string>> {
