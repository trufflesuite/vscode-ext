...
export default async function calculateHash(filePath: string): Promise<string> {
  ...
  const calculateHashProcess =
    await (tryExecuteCommandAsync(undefined, false, RequiredApps.git, calculateHashCommand, `"${filePath}"`).result);

  if (calculateHashProcess.code !== 0) {
    const error = new Error(Constants.openZeppelin.hashCalculationFailed(
      Telemetry.obfuscate(calculateHashProcess.cmdOutputIncludingStderr)));
