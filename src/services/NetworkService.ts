...
export namespace NetworkService {
  ...
  export async function getNetworkMapByNetworkName(networkName: string, ignoreUnidentifiedNetworks: boolean = true)
  ...
  async function getNetworkMap(truffleNetwork: TruffleConfiguration.INetwork, ignoreUnidentifiedNetworks: boolean)
    : Promise<NetworkMap | undefined> {
  ...
  function getTruffleNetworkHost(networkOptions: TruffleConfiguration.INetworkOption): string {
    return `${networkOptions.provider ? networkOptions.provider.url : ''}` ||
